...
def 신규JSON_부모참조형_변환(raw_json: dict) -> dict:
    ...
    for idx, node in enumerate(nodes, start=1):
        결과.append({
            "type" : node["type"],
            "inhouse": bool(node.get("inhouse", False)),
            "행번호": idx,
            "품명": node["id"],                 # 표시용
...
# 📌 양식 박스를 그리는 함수
# -------------------------------------------------------
def 양식박스_그리기(ws, 시작행, 시작열, 품명, 품번, 수량, 재질, type, inhouse=False):
    """
...
def 사양별_시트_생성_및_도식화(
    ...
    for 사양명, 트리 in 사양별_트리.items():
        ...
        실제시트명 = 안전한_시트명(결과통합문서, 시트명)
        ws = 결과통합문서.create_sheet(title=실제시트명)
        사양별_실제시트명[사양명] = 실제시트명
...
# ================================================================
def 트리_연결선_전체_그리기(결과통합문서, 사양별_트리, 시트명_생성함수=None, 사양별_실제시트명=None):
    """
    사양별 트리를 순회하며
    모든 부모–자식 연결선을 그림
    """
    ...
    for 사양명, 트리 in 사양별_트리.items():
        시트명 = (
            사양별_실제시트명.get(사양명)
            if 사양별_실제시트명 and 사양명 in 사양별_실제시트명
            else (시트명_생성함수(사양명, 결과통합문서) if 시트명_생성함수 else str(사양명)[:31])
        )
        ws = 결과통합문서[시트명]
...
def 엑셀_스타일_초기화():
    ...
    global 레벨_이동칸, 박스_높이
    global 두꺼운선, 얇은선, 테두리, 두꺼운테두리, 빨간_두꺼운선, 파란_두꺼운선
    global 가운데정렬, 회색채움
    ...
    파란_두꺼운선 = Side(style="thick", color="1E40AF")
